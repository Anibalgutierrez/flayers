...
const ImagenVisitante = ({ visitorTeamLogo, setVisitorTeamLogo, imagesArray }) => {
  ...
  return (
    <Box>
      <Image src={visitorTeamLogo} alt="Visitor Team Logo" boxSize="100px" onClick={openModal} cursor="pointer" />
          ...
          <ModalBody>
            <SimpleGrid columns={3} spacing={4}>
              {imagesArray.map((image, index) => (
                <Image key={index} src={image} alt={`Logo ${index}`} boxSize="100px" onClick={() => selectImage(image)} cursor="pointer" />
              ))}
            </SimpleGrid>
          </ModalBody>
        </ModalContent>
